...
DELETE FROM feed_follows WHERE id = $1 AND user_id = $2 RETURNING id, created_at, updated_at, user_id, feed_id
`
...
func (q *Queries) DeleteFeedFollows(ctx context.Context, arg DeleteFeedFollowsParams) (FeedFollow, error) {
	row := q.db.QueryRowContext(ctx, deleteFeedFollows, arg.ID, arg.UserID)
	var i FeedFollow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
